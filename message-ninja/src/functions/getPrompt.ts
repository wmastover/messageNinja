...
export const getPrompt =  (url: string, iframe: HTMLIFrameElement): string => {
    ...
    if (url.includes("https://twitter.com")) {
      ...
      const prompt = `Create a casual, non-salesy, one-line (less than 15 words) personalized message for the following 
                      LinkedIn user based on their name, user description, about description, and an array of their experience.
                      Start the message with 'Hey **name**!'. Focus on one key detail: If they have an interesting 
                      'about description' or 'user description', reference this. If not, reference their experience at a 
                      specific job and mention the company name. Avoid controversial topics like politics and asking questions.
                      If no useful information is available, create a friendly greeting. 
